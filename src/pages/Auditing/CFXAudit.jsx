...
export const CFXAudit = () => {
  ...
  const columns = [
    ...
    { headerName: 'Job ID', field: 'job_id', cellStyle: { 'text-align': "center" }, width: 120, cellRenderer: 'viewRenderer' },
    { headerName: 'EDL Run ID', field: 'trgt_edl_run_id', cellRenderer: 'orchestratorRenderer', cellStyle: { 'text-align': "center" }, width: 320 },
    { headerName: 'Inbound Bucket', field: 's3_inbnd_bkt', cellStyle: { 'text-align': "center" }, width: 320 },
    { headerName: 'Inbound Key', field: 's3_inbnd_key', cellStyle: { 'text-align': "center" }, width: 320 },
    { headerName: 'Event File DTLS', field: 'evnt_file_btch_dtls', cellStyle: { 'text-align': "center" }, width: 320 },
    { headerName: 'Event File Name', field: 'evnt_file_nm', cellStyle: { 'text-align': "center" }, width: 320 },
    {
    ...
    },
    {
      ...
      cellStyle: function (params) { return {...CommonFunc.SetColor(params), 'text-align': "center"} }, pinned: 'right', suppressMovable: true,
    },
    {
  ...
  const callAPI = async (env, appCode, dateFilter = undefined) => {
    try {
      ...
      const data = await fetch(getBaseURL(env) + `getauditdetails?env=${env}&app_cd=${appCode}&queryType=cfxAudt${dateFilter ? `&dt_fltr=${dateFilter}` : ''}`)
        .then(x => x.json())
      if (data[0]['count'] !== '0') {
